...
class FitPlot(PairedPlot):
    def __init__(self, field, x_ticker=None, y_ticker=None, figsize=(4.5, 4.5), fontsize=8, orientation="horizontal", **kwargs):
        self.field = field
    ...
    def construct_main_plot(self):
        ...
        plot_fn = [partial(param_map._plot, title="") for param_map in param_maps]
        init_click_coords = param_maps[0].get_worst_coords()
        return DropdownViewPlot(plot_fn=plot_fn, click_fn=self.click, title=self.field.param_names,
                                preserve_clicks_on_view_change=True, init_click_coords=init_click_coords)
