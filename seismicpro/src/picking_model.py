...
def make_prediction():
    ...
    parser.add_argument('-d', '--path_dump', type=str, help="Path to CSV file where \
                        the results would be stored.", default='picking.csv')
    ...
    parser.add_argument('-ts', '--trace_len', type=int, help="The number of first samples \
                        of the trace to load.", default=None)
...
def predict(path_raw, path_model, num_zero=100, save_to='dump.csv',
            batch_size=1000, trace_len=None, device='cpu', shift=0):
    ...
    trace_len: int, default None
        The number of first samples in the trace to load.
        By default loads all the trace samples
    ...
    shift: float, default: 0
        ...
        Set shift = pi / 2 to move first arival from maxima to the 0.
        Set shift = pi  to move first arival from maxima to the minima.
    """
    ...
    test_tmpl = (data.p
                 ...
                 .call(lambda x: np.stack(x.raw), save_to=B('raw'))
