...
def test_dump_single_gather(segy_path, tmp_path, name, copy_header, header_index, header_cols, dump_index):
    ...
    drop_columns = ["TRACE_SEQUENCE_FILE"] + list({"TRACE_SAMPLE_COUNT"} & set(tuple(expected_gather.headers.columns)))
    ...
    if copy_header:
        ...
        sample_rates = full_dump_headers['TRACE_SAMPLE_COUNT']
        assert np.unique(sample_rates) > 1
        assert np.allclose(sample_rates[0] / 1000, expected_survey.sample_rate)
        full_exp_headers.drop(columns=["TRACE_SEQUENCE_FILE", "TRACE_SAMPLE_COUNT"], inplace=True)
        full_dump_headers.drop(columns=["TRACE_SEQUENCE_FILE", "TRACE_SAMPLE_COUNT"], inplace=True)
...
def test_aggregate_segys(segy_path, tmp_path, mode, indices):
    ...
    #TODO: optimize
    drop_columns = ["TRACE_SEQUENCE_FILE"] + list({"TRACE_SAMPLE_COUNT"} & set(tuple(expected_survey.headers.columns)))
