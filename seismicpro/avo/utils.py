...
def aggregate_by_bins_numba(stats, offsets, bin_bounds, limits):
    res = np.empty((len(limits), len(bin_bounds)-1), dtype=np.float32)
    for num in prange(len(limits)):
        start_ix, end_ix = limits[num]
        bin_ixs = np.searchsorted(bin_bounds, offsets[start_ix: end_ix], side="right") - 1
        for ix in prange(len(bin_bounds)):
            bin_stats = stats[start_ix: end_ix][bin_ixs==ix]
